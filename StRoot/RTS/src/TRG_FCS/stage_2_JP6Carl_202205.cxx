...
void  fcs_trg_base::stage_2_JP6Carl_202205(link_t ecal[], link_t hcal[], link_t pres[], geom_t geo, link_t output[])
{    
    int ns=geo.ns;
    ...
    //Carl's 6 square JP
    int e_col_start[3] = { 0, 1, 3};  //these are 2x2 row/col
    int e_col_stop[3]  = { 4, 5, 9};
    int e_row_start[2] = { 0, 5};
    int e_row_stop[2]  = {10,15};
    int h_col_start[3] = { 0, 1, 2};
    int h_col_stop[3]  = { 2, 3, 5};
    int h_row_start[2] = { 0, 3};
    int h_row_stop[2]  = { 6, 9};			  
    int JP1[6] = {0,0,0,0,0,0};
    int JP2[6] = {0,0,0,0,0,0};
    int j=0;
    for(int jc=0; jc<3; jc++){
      for(int jr=0; jr<2; jr++){
	jet[ns][j]=0;
	for(int c=e_col_start[jc]; c<=e_col_stop[jc]; c++){ 
	  for(int r=e_row_start[jr]; r<=e_row_stop[jr]; r++){ 
	    jet[ns][j] += ecal[ETbTdep[r][c]].d[ETbTadr[r][c]]; 
	  }
	}
	for(int c=h_col_start[jc]; c<=h_col_stop[jc]; c++){ 
	  for(int r=h_row_start[jr]; r<=h_row_stop[jr]; r++){ 
	    jet[ns][j] += hcal[HTbTdep[r][c]].d[HTbTadr[r][c]]; 
	  }
	}
        if(jet[ns][j]>0xff) jet[ns][j]=0xff;
        if(jet[ns][j]>JETTHR1) JP1[j] = 1;
        if(jet[ns][j]>JETTHR2) JP2[j] = 1;
	j++;
      }
    }
    if(fcs_trgDebug>=2) printf("JP6C = %3d %3d %3d %3d %3d %3d\n",jet[ns][0],jet[ns][1],jet[ns][2],jet[ns][3],jet[ns][4],jet[ns][5]);
    ...
    output[0].d[4] = (JP1[0] << 0) + (JP1[1]<<1) + (JP1[2]<<2) + (JP1[3]<<3) + (JP1[4]<<4) + (JP1[5]<<5);
    ...
    output[1].d[2] = 0 ;
    output[1].d[3] = 0 ;
    output[1].d[4] = (JP2[0] << 0) + (JP2[1]<<1) + (JP2[2]<<2) + (JP2[3]<<3) + (JP2[4]<<4) + (JP2[5]<<5);
    output[1].d[5] = 0 ;
    output[1].d[6] = 0 ;
    output[1].d[7] = 0xAB ;
